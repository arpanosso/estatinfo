...
O exemplo a seguir apresentará o cálculo da média amostral da idade em anos de $49$ alunos (`idade_anos`). Observe que a média tem as mesmas unidades de medida que as observações individuais. 
...
$\bar{x} = \frac{1,98+1,59+ \cdots+1,78}{49} =  1,724\text{ m}$
...
## Média Ponderada
Em alguams situações as observações têm graus de importância diferentes. Usa-se então a média ponderada:

$$\bar{x}_P = \frac{\sum\limits_{i=1}^{n}x_i \cdot \lambda_i}{\sum\limits_{i=1}^{n}\lambda_i}$$

Onde $\lambda_i$ é o peso associado à i-ésima observaçõe. Assim, ele mede a importância relativa dessa i-ésima observação em relação às demais.
---
...
No Excel, utilizamos a função `=CURT()`.
