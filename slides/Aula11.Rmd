...
Esta expressão, deve ser interpretada do seguinte modo: contruídos todos os intervalos da orma $\bar{X} \pm 1,96 \sigma_{\bar{X}}$, $95\%$ deles conterão $\mu$. Lembrando  que $\mu$ não é uma variável aleatória, mas um parâmetro, isto é, não é o mesmo que dizer que $\mu$ tem $95\%$ de probabilidade de estar entre os limites indicados.
...
$$IC(\mu;95\%) = ]171,70 \pm 1,96 \frac{7,79}{\sqrt{100}} [ = ]170,17\;kg; 173,23\;kg [$$

---

### (b) Caso em que $n$ é grande $(n \ge 30)$ e $\sigma$ desconhecido.

Como n é grande, a substituição de $\sigma$ pelo desvio padrão amostral $(s)$ não afeta apreciavelmente a estimativa de $IC$:
...
A distribuição da razão $t$ é conhecida como **distribuição $t$ de Student** com 
$r = n – 1$ graus de liberdade.
...
$$E(t) = 0 \text{   e   } Var(t) = \frac{r}{r-2} = \frac{n-1}{n-3}$$
...
Calcula-se a média amostral $\bar{x}_a$ do, no exemplo, ganho de peso diário no período.
