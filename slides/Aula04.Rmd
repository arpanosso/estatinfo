...
Para facilitar o processo de contagem poderíamos ordenar os dados por sexo:
```{r, eval=FALSE}
dados_turmas %>% 
  arrange(sexo) +
  View()
...
O ggplot funciona como de camadas represetações gráficas e de formatações, que são adicionadas de acordo com a necessidade do usuário por meio do operador de adição `+` digitado ao final da cada linha.
...
Quando a variável  for quantitativa discreta, os mesmos gráficos de variáveis qualitativas podem ser utilizados. Porém, também recomendamos a utilização dos gráficos `boxplot`, `histograma` e `ogiva percentual`.
...
Para fazer isso basta passarmos como preenchimento o nome da variável categórica.
...
Quando a variável quantitativa for contínua, recomendamos a utilização dos gráficos `boxplot`,`histograma` e `ogiva percentual`. 

Devemos, inicialmente construir a tabela de frequência da variável `altura`. Porém, devemos lwvar em consideração que seus valores não se repetem, pois não sabemos o valor real da observação e sim temos apenas uma aproximação dada pelo instrumento de medida.
